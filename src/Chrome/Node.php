...
class Node
{
    ...
    {
        if (! $this->objectId) {
            $d              = $this->tab->api()->send('DOM.resolveNode', [ 'nodeId' => $this->id ]);
    ...
    public function callFunction($function)
    {
        ...
        return $this->tab->api()->send('Runtime.callFunctionOn', [
    ...
    {
        $bm = $this->tab->api()->send('DOM.getBoxModel', [ 'nodeId' => $this->id ]);

        return Box::fromFlatArray($bm[ 'model' ][ 'content' ]);
    }

    protected function findOrFail($selector)
    {
        return $this->tab->findOrFail($selector, $this->id);
    }
}
